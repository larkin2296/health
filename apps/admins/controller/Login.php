...
//        }
    ...
    public function pwd_email()
    {
        if(input('email'))
        {
            ...
            $url = "www.health.com/pwd_back_email?code=$code&email=$email";
            $title = '医保中心密码找回邮件';
            $data['email'] = $email;
            $message = "尊敬的".input('username')."用户您好:您的密码已经重置请复制后面连接进行重新设置密码。".$url;
    ...
    public function test(){
        $test = \phpmailer\Email::send('ltstudy@sina.com','demoa','测试');
        var_dump($test);
//        $Test = new \test\Test();
//        $Test->test();
    }
     ...
     * 邮箱找回密码
     */
    public function pwd_back_email($code,$email)
    {
